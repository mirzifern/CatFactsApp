        ...
        GeometryReader { geometry in
            ZStack {
                ...
                VStack {
                    ...
                    ScrollView {
                        VStack(spacing: 20) {
                            if catFactsViewModel.state.isLoading {
                                ProgressView("Loading")
                            } else {
                                if let errorMessage = catFactsViewModel.state.errorMessage {
                                    Text(errorMessage)
                                        .font(.headline)
                                    ...
                                    if let image = catFactsViewModel.state.catImage {
                                        Image(uiImage: image)
                                            .resizable()
                                            .scaledToFit()
                                    ...
                                    if let fact = catFactsViewModel.state.catFact {
                                        Text(fact)
                                            .font(.headline)
                                            .multilineTextAlignment(.center)
                                            .foregroundColor(.white)
